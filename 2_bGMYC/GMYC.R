clade.cluster<-function(path.to.input,path.to.output,write.fastas,clust.prob,
  ...
  for(i in 1:length(folders)){
    ...
    y<-grep("fas",files)
    fasta <- ape:::read.dna(files[y], format="fasta")
    fastas<-list()
...
result<-clade.cluster(path.to.input="/Users/herpworld/Desktop/coexp_scripts/dryad/2_bGMYC/input",
                      path.to.output="/Users/herpworld/Desktop/coexp_scripts/dryad/2_bGMYC/output",
                      write.fastas=T,
                      clust.prob=0.5,
                      min.samp = 10,ntrees=100,mcmc=50000,burnin=40000)
